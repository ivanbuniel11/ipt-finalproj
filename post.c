...
void create_json_array(Report info) {
    ...
    json_object_set_new(obj, "title", json_string(info.title));
    json_object_set_new(obj, "author", json_string(info.author));
    json_object_set_new(obj, "genre", json_string(info.genre));
    json_object_set_new(obj, "availability", json_string(info.availability));
...
void loadJSON(Report *info) {  
    ...
    const char* properties[] = { "id", "title", "author", "genre", "availability"};
    char** infovar[] = {&info->id,&info->title,&info->author,&info->genre,&info->availability};
...
int main(void) {
  ...
  while (FCGI_Accept() >= 0) {
   ...
   loadJSON(&info);
    ...
    // bitwise operator
    dbres |= sqlite3_bind_text(stmt, 2, info.title, -1, SQLITE_TRANSIENT);
    dbres |= sqlite3_bind_text(stmt, 3, info.author, -1, SQLITE_TRANSIENT);
      dbres |= sqlite3_bind_text(stmt, 4, info.genre, -1, SQLITE_TRANSIENT);
      dbres |= sqlite3_bind_text(stmt, 5, info.availability, -1, SQLITE_TRANSIENT);
