...
int main() {
 while (FCGI_Accept() >= 0) {
    ...
    rc = sqlite3_open("/var/www/ipt/santiago/cgi-bin/db/data.db", &db);
    ...
    const char *sql = id == 0 ? "SELECT * FROM info WHERE id > ?" : "SELECT * FROM info WHERE id = ?";
    ...
    while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            json_object_set_new(object, "title", json_string((const char*)sqlite3_column_text(stmt, 1)));
            json_object_set_new(object, "author", json_string((const char*)sqlite3_column_text(stmt, 2)));
            json_object_set_new(object, "genre", json_string((const char*)sqlite3_column_text(stmt, 3)));
            json_object_set_new(object, "availability", json_string((const char*)sqlite3_column_text(stmt, 4)));
